  ...
  (fn [ctx imgData]
    (let [w (.-width imgData)
      ...
      (doseq [i (range h)
        ...
        (aset (.-data out) idx (aget orgnl idx_floor))
        (aset (.-data out) idx2 (aget orgnl (+ idx_floor 1)))
        (aset (.-data out) idx3 (aget orgnl (+ idx_floor 2)))
        (aset (.-data out) idx4 (aget orgnl (+ idx_floor 3))))
