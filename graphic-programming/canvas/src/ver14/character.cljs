...
(defn calcNewStatus [h w obj]
  (let [shot_speed (.-speed obj)
    ...
    [ ;; 座標を更新する
     ...
     ;; 枠からはみ出たら、無効にする
     (if (outOfCanvas? h w obj) 0 (.-life obj))]
    ))
...
;; enemy クラス
(defn Enemy [ctx x y imagePath speed [a v] power]
  ;; extend
  (gobj/extend
      (.-prototype Enemy)
      (.-prototype Character))
...
;; shot クラス
(defn Shot [ctx x y imagePath speed [a v] power targets]
  ;; extend
  (gobj/extend
      (.-prototype Shot)
      (.-prototype Character))
...
(set! (.. Position -prototype -distance)
      (fn [target]
        (this-as this
          (let [x (- (.-x this) (.-x target))
                y (- (.-y this) (.-y target))]
      ...
      (fn [h w rotateFlg]
        (this-as this
          ...
          (if (pos? (.-life this))
            (do
              ;; パラメーターを更新
              (.setShot this (calcNewStatus h w this))

              ;; パラメーターを更新２
              (if (pos? (.-life this))
                (doseq [t (.-targetArray this)]
                  (if (pos? (.-life t))
                    (let [dist
                          (.distance
                           (.-position this)
              ...
              (case (.-type this)
                ...
                (do
                  ...
                  (if (= sceneFrameCnt 50)
                    (.fire this this))
                  ;; パラメーターを更新
                  (.setEnemy this (calcNewStatus h w this)))

                ;; case: default==============
                ;;(.log js/console (str "type: " (.-type this)))
                ;;============================
                )
              ;; 敵画像を描画する
              (.draw this)))
