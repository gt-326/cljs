(ns ver11_2.character
  (:require
   [ver11_2.util :as util]
   [goog.object :as gobj]))
...
;; shot クラス
(defn Shot [ctx x y imagePath degree]
  ;; extend
  (gobj/extend
      (.-prototype Shot)
      (.-prototype Character))
  ...
  (this-as this
    ...
    (set! (.-angle this) (util/degToRed degree))
    ;; 自身のフィールド「vector」を設定
    (.setVectorFromDegrees this degree)
...
(set! (.. Shot -prototype -setVectorFromDegrees)
      (fn [degrees]
        (this-as this
          ;; 度数をラジアンに変換する
          (let [radian (util/degToRed degrees)
                sin (Math.sin radian)
                cos (Math.cos radian)]

            ;; 自身の vector プロパティに設定する
            (set! (.-vector this) (Position. cos sin))
            ))))
